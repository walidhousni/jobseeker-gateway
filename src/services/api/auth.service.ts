...
import { IAuth } from "@walidhousni/jobseeker-shared";
...
class AuthService {
   ...
   async sendEmail(data: { userId: number, email: string }): Promise<AxiosResponse> {
   ...
   async getGig(gigId: string): Promise<AxiosResponse> {
      const response: AxiosResponse = await this.axiosService.axios.get(`/search/git/${gigId}`);
      return response;
   }
